...
const DEFAULTS={
 ...
 snakeStyle:"block",foodStyle:"block",graceDozer:true
};
const THEMES={
...
function renderAchievements(){const a=[["Bush camping",!!stats.bushCamping],["Limp",!!stats.limp],["X marks the spot",!!stats.xMarksTheSpot],["Play a different game.",!!stats.playDifferentGame],["Killdozer",!!stats.killdozer],["The achievement you shouldn't have gotten.",!!stats.unwantedAchievement],["First Bite",stats.food>0],["Century",best>=100],["Long Snake",stats.bestLength>=20],["Dedicated",stats.games>=10],["Survivor",stats.bestTime>=120],["Powered Up",stats.powerups>0]];$("achievements").innerHTML=a.map(x=>"<span class='"+(x[1]?"unlocked":"")+"'>"+(x[1]?"★ ":"☆ ")+x[0]+"</span>").join("")}
...
function spawnCurseWall(){if(!running||!cursedState.active)return;const head=snake[0],cells=[];let p={x:head.x,y:head.y};for(let i=0;i<settings.mapSize;i++){p={x:p.x+direction.x,y:p.y+direction.y};if(settings.wrap){p.x=(p.x+settings.mapSize)%settings.mapSize;p.y=(p.y+settings.mapSize)%settings.mapSize}else if(p.x<0||p.x>=settings.mapSize||p.y<0||p.y>=settings.mapSize)break;if(!cells.some(x=>same(x,p)))cells.push(p)}if(!cells.length){scheduleCurseRoll();return}cursedState.wall=cells[rand(cells.length)];$("curseWall").classList.add("show");draw()}
...
function beginDozerWarning(){if(!running||paused||!settings.graceDozer)return;dozerState.warningTimer=null;dozerState.phase="warning";dozerState.warningEnds=performance.now()+3000;stopTimer();updateDozerDisplay();dozerState.warningTimer=setInterval(()=>{updateDozerDisplay();if(performance.now()>=dozerState.warningEnds){clearInterval(dozerState.warningTimer);dozerState.warningTimer=null;dozerState.phase="attack";$("dozerWarning").classList.remove("show");startDozerAttack()}},50)}
...
function nextHead(){let p={x:snake[0].x+direction.x,y:snake[0].y+direction.y};if(settings.wrap){p.x=(p.x+settings.mapSize)%settings.mapSize;p.y=(p.y+settings.mapSize)%settings.mapSize}else if(powerState.killdozerUntil&&performance.now()<powerState.killdozerUntil){if(p.x<0||p.x>=settings.mapSize)direction.x*=-1;if(p.y<0||p.y>=settings.mapSize)direction.y*=-1;p={x:snake[0].x+direction.x,y:snake[0].y+direction.y}}return p}
...
function draw(){clearBoard();if(!feature.reducedMotion)animateBackground(performance.now());drawGrid();drawObstacles();if(cursedState.wall){const c=cellSize();ctx.fillStyle="#fff200";ctx.fillRect(cursedState.wall.x*c,cursedState.wall.y*c,c,c);ctx.strokeStyle="#fff";ctx.lineWidth=Math.max(1,c*.06);ctx.strokeRect(cursedState.wall.x*c+.5,cursedState.wall.y*c+.5,c-1,c-1)}drawFood();drawPowerups();drawSnake();drawParticles();if(flash>0){ctx.globalAlpha=flash*.12;ctx.fillStyle="#fff";ctx.fillRect(0,0,canvas.width,canvas.height);ctx.globalAlpha=1;flash=Math.max(0,flash-.08)}}
...
function bindAll(){
 ...
 ["mapSize","wrap","selfCollision","grid","ghostFood","respawn","nearMiss","perfectBonus","theme","snakeStyle","foodStyle","graceDozer"].forEach(id=>bindSetting(id,false));
...
function setupSecretCode(){const input=$("codeInput"),limp=$("limpOverlay"),restart=$("limpRestart");if(!input||!limp||!restart)return;input.addEventListener("keydown",e=>{if(e.key!=="Enter")return;e.preventDefault();const code=input.value.trim().toLowerCase();if(code==="limp")triggerLimp();if(code==="1987"){unlockAchievement("unwantedAchievement","The achievement you shouldn't have gotten.");flashImage(CURSE_START_IMAGE);startCurse()}input.value=""});restart.addEventListener("click",()=>{limp.classList.remove("show");limp.setAttribute("aria-hidden","true");newGame()})}

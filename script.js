...
function bindAll(){
 ...
 $("gameMode").onchange=e=>{feature.mode=e.target.value;applyMode();applySettingsToUI();featureSave();saveSettings();newGame()};$("powerupsEnabled").onchange=e=>{if(feature.mode==="hard"){e.target.checked=true;return}feature.powerups=e.target.checked;featureSave()};$("powerupRate").oninput=e=>{if(feature.mode==="hard"){e.target.value=60;feature.powerupRate=60;$("powerupRateVal").textContent="60%";return}feature.powerupRate=+e.target.value;$("powerupRateVal").textContent=e.target.value+"%";featureSave()};$("soundEnabled").onchange=e=>{feature.sound=e.target.checked;featureSave()};$("musicEnabled").onchange=e=>{toggleMusic(e.target.checked);featureSave()};$("vibrationEnabled").onchange=e=>{feature.vibration=e.target.checked;featureSave()};$("volume").oninput=e=>{feature.volume=+e.target.value;$("volumeVal").textContent=e.target.value+"%";featureSave()};["reducedMotion","largeUI","highContrast"].forEach(id=>$(id).onchange=e=>{feature[id]=e.target.checked;document.body.classList.toggle(id==="largeUI"?"large-ui":id==="highContrast"?"high-contrast":"reduced-motion",e.target.checked);featureSave()});

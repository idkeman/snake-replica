...
const DEFAULTS={
 ...
 snakeStyle:"block",foodStyle:"block",graceDozer:true,ventHunter:false
};
const THEMES={
...
function ventHunterTick(){if(!settings.ventHunter||!running)return;if(ventState.phase==="hidden"){if(!ventState.timer)ventState.timer=setTimeout(()=>{ventState.timer=null;spawnVentHunter()},3000);return}if(ventState.phase==="chase"){if(performance.now()>=ventState.chaseEnds){const target=nearestVent(ventState.hunter);ventState.returnPath=target?bfsPath(ventState.hunter,target):[];ventState.returnIndex=0;ventState.phase="return";return}ventState.hunter=stepHunter(ventState.hunter,predictedTarget());if(same(ventState.hunter,snake[0])){flashImage(VENT_HUNTER_HIT_IMAGE);gameOver("SHE GOT YOU");return}}else if(ventState.phase==="return"){if(ventState.returnIndex<ventState.returnPath.length)ventState.hunter=ventState.returnPath[ventState.returnIndex++];else{ventState.hunter=null;ventState.phase="hidden";ventState.timer=setTimeout(()=>{ventState.timer=null;spawnVentHunter()},3000)}}}
...
function setupSecretCode(){const input=$("codeInput"),limp=$("limpOverlay"),restart=$("limpRestart");if(!input||!limp||!restart)return;input.addEventListener("keydown",e=>{if(e.key!=="Enter")return;e.preventDefault();const code=input.value.trim().toLowerCase();if(code==="limp")triggerLimp();if(code==="1987"){unlockAchievement("unwantedAchievement","The achievement you shouldn't have gotten.");flashImage(CURSE_START_IMAGE);startCurse()}if(code==="1983"){unlockAchievement("itsAYes","It's a yes.");settings.ventHunter=true;saveSettings();applySettingsToUI();toast("VENT HUNTER UNLOCKED")}input.value=""});restart.addEventListener("click",()=>{limp.classList.remove("show");limp.setAttribute("aria-hidden","true");newGame()})}

...
function randomize(){const choices={mapSize:[15,20,25,30,35,40][rand(6)],foodCount:rand(12)+1,foodValue:rand(8)+1,startLength:clamp(rand(10)+2,2,20),speed:35+rand(30)*5,speedGrowth:rand(6)*2,wrap:Math.random()<.35,selfCollision:Math.random()<.85,grid:Math.random()<.5,ghostFood:Math.random()<.35,respawn:true,obstacles:rand(16),bonusChance:rand(61),goldMultiplier:2+rand(6),nearMiss:Math.random()<.5,perfectBonus:Math.random()<.5,theme:Object.keys(THEMES)[rand(4)],snakeStyle:["block","dot","outline","scan"][rand(4)],foodStyle:["block","dot","cross","diamond"][rand(4)]};settings={...settings,...choices};normalizeSettings();applySettingsToUI();saveSettings();newGame();toast("Random modifier set generated")}
...
function drawFood(){const t=THEMES[settings.theme],c=cellSize(),now=performance.now()/160;foods.forEach(f=>{if(settings.ghostFood&&Math.sin(now+f.phase)<-.25)return;const x=f.x*c,y=f.y*c;ctx.fillStyle=f.differentGame?"#b36bff":f.bushCamping?"#198754":f.bonus?t.bonus:t.food;const inset=c*.18;if(f.differentGame){ctx.fillRect(x+c*.18,y+c*.18,c*.64,c*.64);ctx.fillStyle=t.bg;ctx.font=Math.max(8,c*.42)+"px monospace";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("♟",x+c/2,y+c*.53);return}if(f.bushCamping){ctx.beginPath();ctx.arc(x+c/2,y+c*.38,c*.22,0,Math.PI*2);ctx.arc(x+c*.32,y+c*.58,c*.2,0,Math.PI*2);ctx.arc(x+c*.68,y+c*.58,c*.2,0,Math.PI*2);ctx.fill();ctx.fillRect(x+c*.43,y+c*.48,c*.14,c*.3);return}if(settings.foodStyle==="dot"){ctx.beginPath();ctx.arc(x+c/2,y+c/2,c*.28,0,Math.PI*2);ctx.fill()}else if(settings.foodStyle==="cross"){ctx.fillRect(x+c*.35,y+c*.12,c*.3,c*.76);ctx.fillRect(x+c*.12,y+c*.35,c*.76,c*.3)}else if(settings.foodStyle==="diamond"){ctx.beginPath();ctx.moveTo(x+c/2,y+inset);ctx.lineTo(x+c-inset,y+c/2);ctx.lineTo(x+c/2,y+c-inset);ctx.lineTo(x+inset,y+c/2);ctx.closePath();ctx.fill()}else ctx.fillRect(x+inset,y+inset,c-inset*2,c-inset*2)})}
...
document.addEventListener("keydown",e=>{
 if(e.target&&e.target.matches&&e.target.matches("input,select,textarea,[contenteditable=true]"))return;

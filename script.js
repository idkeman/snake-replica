...
const DEFAULTS={
 mapSize:20,foodCount:3,foodValue:1,startLength:3,speed:100,speedGrowth:0,
...
let stats={games:0,deaths:0,food:0,bestLength:0,bestTime:0,powerups:0,bushCamping:false,limp:false,xMarksTheSpot:false,playDifferentGame:false,killdozer:false,unwantedAchievement:false,itsAYes:false,badassBubbleBass:false};
...
function renderAchievements(){const a=[["Badass Bubble Bass",!!stats.badassBubbleBass],["Bush camping",!!stats.bushCamping],["Limp",!!stats.limp],["X marks the spot",!!stats.xMarksTheSpot],["Play a different game.",!!stats.playDifferentGame],["Killdozer",!!stats.killdozer],["The achievement you shouldn't have gotten.",!!stats.unwantedAchievement],["It's a yes.",!!stats.itsAYes],["First Bite",stats.food>0],["Century",best>=100],["Long Snake",stats.bestLength>=20],["Dedicated",stats.games>=10],["Survivor",stats.bestTime>=120],["Powered Up",stats.powerups>0]];$("achievements").innerHTML=a.map(x=>"<span class='"+(x[1]?"unlocked":"")+"'>"+(x[1]?"★ ":"☆ ")+x[0]+"</span>").join("")}
...
function specialFoodFlash(url){const el=$("specialFlash");if(!el)return;stopTimer();paused=true;el.src=url;el.style.display="block";el.style.opacity="1";el.style.transition="none";void el.offsetWidth;requestAnimationFrame(()=>{el.style.transition="opacity 1000ms linear";el.style.opacity="0"});setTimeout(()=>{el.style.display="none";el.style.opacity="0";el.style.transition="none";paused=false;gameStart=performance.now()-elapsed*1000;startTimer()},1000)}
...
function randomize(){const mapSizes=[15,20,25,30,35,40];const mapSize=mapSizes[rand(mapSizes.length)];const startLength=Math.min(rand(10)+2,mapSize);const maxObstacles=Math.max(0,mapSize*mapSize-startLength-12);const foodCount=Math.min(rand(12)+1,Math.max(1,mapSize*mapSize-startLength-Math.min(rand(16),maxObstacles)));const choices={mapSize,foodCount,foodValue:rand(8)+1,startLength,speed:35+rand(30)*5,speedGrowth:rand(6)*2,wrap:Math.random()<.35,selfCollision:Math.random()<.85,grid:Math.random()<.5,ghostFood:Math.random()<.35,respawn:true,obstacles:Math.min(rand(16),maxObstacles),bonusChance:rand(61),goldMultiplier:2+rand(6),nearMiss:Math.random()<.5,perfectBonus:Math.random()<.5,theme:Object.keys(THEMES)[rand(4)],snakeStyle:["block","dot","outline","scan"][rand(4)],foodStyle:["block","dot","cross","diamond"][rand(4)]};settings={...settings,...choices};applySettingsToUI();saveSettings();newGame();toast("Random modifier set generated")}
...
function randomOpenCell(){const total=settings.mapSize*settings.mapSize;for(let tries=0;tries<total*2;tries++){const p={x:rand(settings.mapSize),y:rand(settings.mapSize)};if(cellAvailable(p))return p}for(let y=0;y<settings.mapSize;y++)for(let x=0;x<settings.mapSize;x++){const p={x,y};if(cellAvailable(p))return p}return null}
...
function spawnFood(forceBonus=false,forceDifferent=false){
 ...
 if(!forceBonus&&!forceDifferent&&Math.floor(Math.random()*150)===0){foods.push({x:p.x,y:p.y,bonus:false,value:settings.foodValue,phase:Math.random()*Math.PI*2,bubbleBass:true});return true}
...
function fillFood(){const target=Math.min(settings.foodCount,settings.mapSize*settings.mapSize-1);let attempts=0;while(foods.length<target&&attempts++<target*4+10){if(!spawnFood())break}}
...
function nextHead(){let p={x:snake[0].x+direction.x,y:snake[0].y+direction.y};const killdozer=powerState.killdozerUntil&&performance.now()<powerState.killdozerUntil;if(killdozer){let bounced=false;if(p.x<0||p.x>=settings.mapSize){direction.x*=-1;bounced=true}if(p.y<0||p.y>=settings.mapSize){direction.y*=-1;bounced=true}if(bounced){queued={x:direction.x,y:direction.y};inputQueue=[];p={x:snake[0].x+direction.x,y:snake[0].y+direction.y}}}else if(settings.wrap){p.x=(p.x+settings.mapSize)%settings.mapSize;p.y=(p.y+settings.mapSize)%settings.mapSize}return p}
...
function eatAt(p){const eaten=foods.filter(f=>same(f,p));if(!eaten.length)return 0;let gained=0;eaten.forEach(f=>{gained+=f.value;burst(f.x,f.y,f.bonus?12:7,f.bonus);if(f.bubbleBass){ctx.fillStyle="#c9823b";ctx.beginPath();ctx.arc(x+c*.5,y+c*.4,c*.28,Math.PI,0);ctx.fill();ctx.fillRect(x+c*.22,y+c*.4,c*.56,c*.28);ctx.fillStyle="#8b451f";ctx.fillRect(x+c*.25,y+c*.64,c*.5,c*.12);return}if(f.killdozer){powerState.killdozerUntil=performance.now()+15000;unlockAchievement("killdozer","Killdozer");toast("KILLDOZER! 5× SPEED · WALL BOUNCE");restartTimerIfNeeded()}});foods=foods.filter(f=>!same(f,p));return gained}
...
function bindAll(){
 ...
 $("devSpawnBubbleBass").onclick=devSpawnBubbleBass;
 ...
 $("mapSize").addEventListener("change",()=>{readSettings();saveSettings();newGame()});["wrap","selfCollision","grid","ghostFood","respawn","nearMiss","perfectBonus","theme","snakeStyle","foodStyle","graceDozer","curseEnabled"].forEach(id=>bindSetting(id,false));
